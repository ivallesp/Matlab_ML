...
for n=1:EXPERIMENTOS
    ...
    for k=1:FOLDS
        ...
        for i=1:length(NEURONAS)
            [salida_elm_t,salida_elm_g]=elm(inputs,targets,trainIndex,testIndex,NEURONAS(i));
            [vpp,vpn,se,sp,v_cutoff,area_t(i,k),W,EE,va,vb,vc,vd]=roc_j(salida_elm_t,DeseadaTr);
            [vpp,vpn,se,sp,v_cutoff,area_g(i,k),W,EE,va,vb,vc,vd]=roc_j(salida_elm_g,DeseadaGe);
